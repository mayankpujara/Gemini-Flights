...
def book_flights(**params):
    """
    Sends a POST request to a FastAPI endpoint to book flights according to the requirments of the user.

    Parameters: 
    - criteria (FlightBookCriteria): An object containing the booking criteria
    Returns:
    - The response from the FastAPI endpoint as a JSON object.
    """
    ...
    # Constructing the URL with query parameters
    url = f"http://127.0.0.1:8000//book_flight/?flight_number={criteria.flight_number}&seat_type={criteria.seat_type}&num_seats={criteria.num_seats}"
